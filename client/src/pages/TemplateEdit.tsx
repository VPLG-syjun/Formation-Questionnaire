...
export default function TemplateEdit() {
  ...
  const handleScanVariables = async () => {
    ...
    try {
      ...
      // API에서 반환한 자동 생성 변수 목록
      const autoGenerated: string[] = data.autoGenerated || [];

      // 자동 생성 변수 중 기존 목록에 없는 것
      const newAutoGenerated = autoGenerated.filter(name => !existingNames.has(name));
      ...
      if (newVariables.length > 0) {
        messages.push(`${newVariables.length}개의 새 변수가 추가되었습니다.`);
      }

      // 자동 생성 변수가 있으면 추가 여부 확인
      if (newAutoGenerated.length > 0) {
        const addAutoGen = window.confirm(
          `${newAutoGenerated.length}개의 자동 생성 변수가 감지되었습니다:\n` +
          `${newAutoGenerated.join(', ')}\n\n` +
          `이 변수들에 데이터 타입/변환 규칙을 지정하시겠습니까?\n` +
          `(예: Founder1Cash에 $ 표시 추가)`
        );

        if (addAutoGen) {
          const autoGenVariables = newAutoGenerated.map(name => ({
            variableName: name,
            questionId: '__auto_generated__',
            dataType: name.toLowerCase().includes('cash') ? 'currency' : 'text',
            transformRule: name.toLowerCase().includes('cash') ? 'comma_dollar' : 'none',
            required: false,
          }));
          setVariables(prev => [...prev, ...autoGenVariables]);
          messages.push(`${newAutoGenerated.length}개의 자동 생성 변수가 추가되었습니다.`);
        }
      }
