  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'scan' && templateId) {
        ...
        const { includeAutoGenerated } = req.body;

        // 자동 생성 변수와 일반 변수 분리
        const autoGenerated: string[] = [];
        const regularVariables: string[] = [];
        allScannedVariables.forEach(v => {
          if (isAutoGeneratedVariable(v)) {
            autoGenerated.push(v);
          } else {
            regularVariables.push(v);
          }
        });

        // includeAutoGenerated가 true면 모든 변수 반환
        const scannedVariables = includeAutoGenerated
          ? allScannedVariables
          : regularVariables;
        ...
        return res.status(200).json({
          ...
          autoGenerated,  // 자동 생성 변수 목록 (별도 표시용)
