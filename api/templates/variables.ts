...
/**
 * 반복 그룹에서 자동 생성되는 변수인지 확인
 */
function isAutoGeneratedVariable(varName: string): boolean {
  // 1. 루프 컨텍스트 필드 (반복문 내부에서만 사용)
  if (LOOP_CONTEXT_FIELDS.includes(varName)) return true;
  ...
  for (const group of REPEATABLE_GROUPS) {
    ...
    // {group}Count - directorsCount, foundersCount
    if (varName === `${group}Count`) return true;

    // hasMultiple{Group}, hasSingle{Group} - hasMultipleDirectors
    if (varName === `hasMultiple${capitalized}` || varName === `hasSingle${capitalized}`) return true;

    // {group}{Field}Formatted, {group}{Field}List, {group}{Field}OrList - directorsNameFormatted
    for (const field of REPEATABLE_GROUP_FIELDS) {
      const fieldCap = field.charAt(0).toUpperCase() + field.slice(1);
      if (varName === `${group}${fieldCap}Formatted` ||
          varName === `${group}${fieldCap}List` ||
          varName === `${group}${fieldCap}OrList`) return true;
    }

    // {singular}{N}{Field} - director1Name, founder2Email (N은 1-9)
    const individualPattern = new RegExp(`^${singular}[1-9](Name|Address|Email|Type|CeoName|Cash)$`, 'i');
    if (individualPattern.test(varName)) return true;

    // {group}{N}{Field} - directors1Name, founders2Cash (새 형식, N은 1-9)
    const groupIndexPattern = new RegExp(`^${group}[1-9](Name|Address|Email|Type|CeoName|Cash)$`, 'i');
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'scan' && templateId) {
        ...
        const scannedVariables = allScannedVariables.filter(v => {
          if (isAutoGeneratedVariable(v)) {
            autoGenerated.push(v);
            return false;
          }
          return true;
        });
        ...
        return res.status(200).json({
          ...
          autoGenerated,  // 자동 생성 변수 목록도 반환 (참고용)
      ...
      if (templateId && Array.isArray(variables)) {
        ...
        // 새 변수 저장 (자동 생성 변수는 건너뜀)
        const savedVariables = [];
        const skippedAutoGenerated: string[] = [];
        for (const v of variables) {
          // 자동 생성 변수는 저장하지 않음
          if (isAutoGeneratedVariable(v.variableName)) {
            skippedAutoGenerated.push(v.variableName);
            continue;
          }
        ...
        return res.status(200).json({
          message: `${savedVariables.length}개의 변수가 저장되었습니다.` +
            (skippedAutoGenerated.length > 0 ? ` (${skippedAutoGenerated.length}개의 자동 생성 변수는 건너뜀)` : ''),
